...
All crypto blobs are base64 (STANDARD). `UniSSH-Tenant: <base64(tenant_id)>` required
on all `/v1` routes except bootstrap-time; `Authorization: Bearer` on private routes.
Mutating routes accept `Idempotency-Key`.

- **identity/auth:** `POST /v1/bootstrap`, `/v1/register`, `/v1/invite`,
  `/v1/invite/redeem`, `/v1/auth/challenge`, `/v1/auth/verify`,
  `/v1/session/{refresh,logout,device-revoke}`, `GET|PUT /v1/keyset`,
  `/v1/relay/{open,msg1,msg2,msg3}` + `GET /v1/relay/poll`.
- **accounts/devices/admin:** `POST /v1/devices/add` (sibling device, shared keyset),
  `GET /v1/accounts` (admin: handles/display names/member-ids/device counts),
  `POST /v1/admin/set` (instance-admin promote/demote), `POST /v1/account/profile`.
  Identity model + client flows: see [`CLIENT.md`](CLIENT.md). An **account = one
  keyset identity** (its Ed25519 = canonical member-id); devices share it; optional
  `display_name`/`handle` are **server-visible** human labels. Instance-admin
  (server-trusted: invites/audit/device-revoke/grants-publish) is decoupled from the
  cryptographic per-vault role.
- **sync:** `POST /v1/sync/push`, `GET /v1/sync/delta`, `GET /v1/sync/version`.
- **vaults/policy:** `POST /v1/vaults/claim`, `POST /v1/grants/publish`,
  `GET /v1/grants`.
- **audit:** `POST /v1/audit`, `GET /v1/audit` (admin).
- **admin/ops** (Bearer-admin, per-tenant, **suspended-gate-exempt** so a suspended
  tenant stays recoverable): `GET /v1/admin/{overview,devices,sessions,invites,
  vaults,vault,objects,relay,keysets,config,migrations}` and
  `POST /v1/admin/{tenant/status,account/status,session/revoke,invite/revoke,
  seq-bump}`. Read-projections of **open metadata** + lifecycle controls for the
  self-hosted admin panel; never exposes ciphertext (object_bytes / keyset_bytes /
  relay messages). `config` is read-only with secrets masked. Account-disable is
  enforced in the auth path (existing sessions stop) with genesis/last-admin
  anti-lockout.
...
`server_seq` is a per-tenant monotonic counter; each client keeps a trusted
last-seen cursor **outside** the server. The anti-rollback invariant requires
...
**`MAX(next_seq)` per tenant** snapshotted to a tiny out-of-band durable key (more
...
- **Metadata leaks by design** (ARCH §5.4): vault/item ids, versions, tombstones,
  ...
  full signed (unencrypted) manifest member-set, blob sizes, push/pull timings. The
...
- **Audit v1: authenticity + tamper-evidence (§11.2).** Client-signed entries are
  authentic (genesis-owner signature). The audit log is now a **server-side hash-chain**
  (`prev_hash` = SHA-256(prev ‖ canonical(record)), computed under the per-tenant write
...
- **Server-side record-signature verification (§2.4) is implemented** behind
  ...
  are gated by `author == genesis_owner`; Keyset blobs are AEAD-authenticated.
